...
def log_message(message, hmac, timestamp, user):
...
def start_server():
    ...
    save_rsa_key_to_file(server_key, "server_private_key.pem")
    save_rsa_key_to_file(server_key.publickey(), "server_public_key.pem")
...
def server_send_messages(conn, key, hmac_key):
    while True:
        ...
        log_message(message, hmac, timestamp, "server sends: ")
...
def start_client():
    ...
    save_rsa_key_to_file(client_key, "client_private_key.pem")
    save_rsa_key_to_file(client_key.publickey(), "client_public_key.pem")
...
def client_send_messages(client_socket, key, hmac_key):
    while True:
        ...
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_message(message, hmac, timestamp, "client sends: ")
        log_encrypted_message(encrypted_message)
